if object_id('percReport_TimelyContentAging') is not null
	drop procedure dbo.percReport_TimelyContentAging
go
create procedure dbo.percReport_TimelyContentAging (@starttime datetime , @endtime datetime, @contentid int = NULL)
as
BEGIN
	select b.contentid, b.title
			, owner_id, ownertype
		, case when ownertype like '%timely%' then ownertitle else
								...
								(			
										...
										and r.dependent_id in 
											(select contentid from contentstatus c inner join contenttypes t on c.contenttypeid = t.contenttypeid 
													where contenttypename = 'cgvTimelyContentBlock')
