...
BEGIN	
	...
	*/	
	...
	SELECT @ptrval = TEXTPTR([XML]) from CancerGovStaging..ListXML where nciviewid=@NCIviewID
	WRITETEXT  CancerGovStaging..ListXML.[XML]  @ptrval @Result
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @ptrval = TEXTPTR([XML]) from  CancerGovStaging..ListXML where nciviewid=@NCIviewID
			UPDATETEXT   CancerGovStaging..ListXML.[XML]  @ptrval NULL 0 @Result
	...
	--	select @Result = @Result +   '</items></item></items></domain>'
		...
		SELECT @ptrval = TEXTPTR([XML]) from  CancerGovStaging..ListXML where nciviewid=@NCIviewID
		UPDATETEXT   CancerGovStaging..ListXML.[XML]  @ptrval NULL 0  '</items></item></items></domain>'
