IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[udf_GetModalityTreeForPDQModalities]') AND xtype in (N'FN', N'IF', N'TF'))
DROP FUNCTION [dbo].[udf_GetModalityTreeForPDQModalities]
GO
CREATE FUNCTION dbo.udf_GetModalityTreeForPDQModalities 
	( 
...
BEGIN
	...
	BEGIN
		...
		SELECT DISTINCT ModalityID 
		FROM PDQModalityTree
		WHERE PDQModalityID IN (
							SELECT ObjectID
