IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[usp_TransformWYNTK]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_TransformWYNTK]
GO
SET ANSI_NULLS OFF
GO
...
/****** 	Object:  Stored Procedure dbo.usp_TransformOESI
	Purpose: This script  is used to transform doc_wyntkI to doc_img. 
	Script Date: 04/16/2004 10:43:49 pM 
******/

CREATE PROCEDURE dbo.usp_TransformWYNTK
AS
BEGIN
	SET NOCOUNT ON;
	...
	SELECT 	@OldNCITemplateID =  ncitemplateID from CancerGovStaging..NCITemplate where [name] like '%doc_wyntk%'
	...
	DECLARE Redirect_Cursor CURSOR LOCAL FORWARD_ONLY  FOR
		...
		BEGIN
			...
			set 	ncitemplateID = @NewNCITemplateID,
			    	URL = REPLACE(@URL, 'doc_wyntk', 'doc_img')
			...
			update  PrettyURL
			set 	RealURL = REPLACE(RealURL, 'doc_wyntk', 'doc_img')
			where nciviewid = @NCIViewID
			IF (@@ERROR <> 0)
			BEGIN
				DEALLOCATE  Redirect_Cursor 
			...
			values
			(@ObjectID, @NCIViewID, 'HDRDOC', 0, 'BatchUpdateWYNTK')
			IF (@@ERROR <> 0)
			BEGIN
				DEALLOCATE  Redirect_Cursor 
				ROLLBACK TRAN  Tran_TransformOESI
