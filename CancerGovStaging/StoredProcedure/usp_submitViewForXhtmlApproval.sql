IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[usp_submitViewForXhtmlApproval]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_submitViewForXhtmlApproval]
GO
SET ANSI_NULLS ON
GO
...
**
*/
CREATE PROCEDURE [dbo].[usp_submitViewForXhtmlApproval] 
	(
...
BEGIN	
	...
	*/
	...
	select @TaskID = newid(), @type=2 -- predefined type for Xhtml review
	...
	*/	
	...
	if (exists (select 1 from [CancerGovStaging].[dbo].[ViewProperty]
		where [NCIViewID] = @viewObj and [PropertyName] ='XHTMLDenied'))
	BEGIN
		delete from [CancerGovStaging].[dbo].[ViewProperty]
		where [NCIViewID] = @viewObj and [PropertyName] ='XHTMLDenied'
	END		
	...
	*/
	insert into Task
	(TaskID, ResponsibleGroupID, Name, UpdateDate, UpdateUserID, ObjectID, status, type)
	values
	(@TaskID, @GroupID, 'Review XHTML conversion of ' + substring(@stitle,1,40) +'...', getdate(), @UpdateUserID, @viewObj, 'Executing', @type)
	IF (@@ERROR <> 0)
	BEGIN
		ROLLBACK TRAN Tran_SubmitViewApproval
		RAISERROR ( 70004, 16, 1)
	...
	*/
	...
	(StepID, ArgumentOrdinal, ForProcedureX, ArgumentValue)
	values
	(@StepID, '3', '1', 'The Page ' + @title + ' needs approval for XHTML conversion.')
	IF (@@ERROR <> 0)
	BEGIN
		ROLLBACK TRAN Tran_SubmitViewApproval
		RAISERROR ( 70004, 16, 1)
	...
	END
		...
		'">' + @title + '</a> has been converted to XHTML and submitted for Migration to the production site, and awaits your approval. ' + 
	...
	*/	
		...
		+ @realPreviewurl    + '">' + @title + '</a> has been submitted for Migration to the production site, and awaits your approval.')
		IF (@@ERROR <> 0)
		BEGIN
			ROLLBACK TRAN Tran_SubmitViewApproval
			RAISERROR ( 70004, 16, 1)
	...
	*/
	...
	END
		...
		'">' + @title + '</a> failed during its migration/ publishing to production after XHTML conversion.')
		IF (@@ERROR <> 0)
		BEGIN
			ROLLBACK TRAN Tran_SubmitViewApproval
			RAISERROR ( 70004, 16, 1)
	...
	*/
	...
	END
		...
		'">' + @title + '</a> with XHTML conversion has been Migrated to the production site successfully.')
		IF (@@ERROR <> 0)
		BEGIN
			ROLLBACK TRAN Tran_SubmitViewApproval
			RAISERROR ( 70004, 16, 1)
...
GO
GRANT EXECUTE ON [dbo].[usp_submitViewForXhtmlApproval] TO [webadminuser_role]
GO
