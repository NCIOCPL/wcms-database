IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[usp_disApproveReferenceDelete]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[usp_disApproveReferenceDelete]
GO
SET ANSI_NULLS OFF
GO
...
**  to delete a view.  This is realy another page owner balking at removing a 
**  link to this page.  We may not want to make this a critical Task Failure.
**
**  NOTE: 
**
**  Issues:  
...
**
*/
CREATE PROCEDURE [dbo].[usp_disApproveReferenceDelete] 
	(
...
BEGIN	
	...
	*/	
	...
	BEGIN TRAN Tran_DisAproveViewDeleteRef
	...
	IF (@@ERROR <> 0)
	BEGIN
		ROLLBACK TRAN Tran_DisAproveViewDeleteRef
...
*/	
	...
	values
	(@sendID, @receiveID, 'View Delete Approval Request Denied', 'View Delete Approval Request Denied, because a reference to this page should not be removed per owning group of that page. View ' + 
	...
	COMMIT TRAN Tran_DisAproveViewDeleteRef
...
END
GO
GRANT EXECUTE ON [dbo].[usp_disApproveReferenceDelete] TO [webadminuser_role]
GO
