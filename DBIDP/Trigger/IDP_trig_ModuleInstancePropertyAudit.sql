IF NOT EXISTS (SELECT * FROM sys.triggers WHERE object_id = OBJECT_ID(N'[dbo].[IDP_trig_ModuleInstancePropertyAudit]'))
EXEC dbo.sp_executesql @statement = N'begin     if @@rowcount = 0 return;
if (exists(select * from inserted) and not exists (select * from deleted)) 
insert into dbo.auditModuleInstanceProperty( InstancePropertyID,ModuleInstanceID,PropertyTemplateID,CreateUserID,CreateDate,UpdateUserID,UpdateDate,PropertyValue,AuditActionType       )     select InstancePropertyID,ModuleInstanceID,PropertyTemplateID,CreateUserID,CreateDate,UpdateUserID,UpdateDate,PropertyValue,  ''I''  as AuditActionType     from inserted
else
