...
BEGIN
	...
	--**************************************************************************************************************
	if @isDebug =1  PRINT  '           *****************************************************************************'
	if @isDebug =1  PRINT  '           START - Push PoliticalSubUnit data from PREVIEW to LIVE '

	IF @CleanOldData = 'Yes' 
	BEGIN
		EXEC @r = CDRLiveGK..usp_ClearExtractedPoliticalSubUnitData @DocumentID
		IF (@@ERROR <> 0) or (@r <>0)
		BEGIN
	...
	INSERT INTO CDRLiveGK..PoliticalSubUnit 
		(
		CountryID, 
		CountryName, 
		FullName, 
	...
	SELECT 	CountryID, 
		...
		@UpdateDate, 
		@UpdateUserID 
	FROM 	CDRPreviewGK..PoliticalSubUnit 
