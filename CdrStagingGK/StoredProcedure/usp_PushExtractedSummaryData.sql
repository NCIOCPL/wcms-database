...
BEGIN
	...
	IF @CleanOldData = 'Yes' 
	BEGIN
		EXEC @r = CDRPreviewGK.dbo.usp_ClearExtractedSummaryData @DocumentID
		IF (@@ERROR <> 0) or ( @r <> 0)
		BEGIN
	...
	INSERT INTO CDRPreviewGK.dbo.Summary
		(
		SummaryID,
		Type,
		Audience,
	...
	INSERT INTO CDRPreviewGK.dbo.SummaryRelations
		(
		SummaryID,
		RelatedSummaryID,
		RelationType,
	...
	SELECT 	SummaryID,
		...
		@UpdateDate,
		@UpdateUserID
	FROM 	CDRStagingGK.dbo.SummaryRelations
	WHERE 	SummaryID = @DocumentID 	
	...
	INSERT INTO CDRPreviewGK.dbo.SummarySection
		(
			SummarySectionID, 
			SummaryGUID, 
			SummaryID, 
	...
	SELECT 	SummarySectionID, 
			...
			TOC,
			[HTML]
	FROM 	CDRStagingGK.dbo.SummarySection
	WHERE 	SummaryID = @DocumentID 	
	...
	if @isDebug =1  PRINT  '           DONE - Push data to Summary and related tables from staging to Preview'
